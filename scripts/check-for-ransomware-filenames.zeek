...
export {
	redef enum Notice::Type += { Ransomware::KnownBadFilename };
}
...
event zeek_init()
	{
	Input::add_table([ $source="fsrm_patterns_for_zeek.tsv",
	    $name="ransomware_patterns", $idx=Idx, $val=Val,
	    $destination=ransomware_filename_patterns_table, $mode=Input::REREAD ]);
...
event Input::end_of_data(name: string, source: string)
	...
	for ( idx in ransomware_filename_patterns_table )
		{
		ransomware_filename_patterns_vector +=
		    ransomware_filename_patterns_table[idx]$rw_pattern;
		}
	# Initialize the paraglob structure
	ransomware_filename_patterns_paraglob = paraglob_init(
	    ransomware_filename_patterns_vector);
	}
...
event Files::log_files(rec: Files::Info)
	...
	local num_matches = |paraglob_match(ransomware_filename_patterns_paraglob,
	    rec$filename)|;
...
@if ( Version::info$major >= 5.1 )
	# Handle the v5 files log
	# see if there were any matches
	if ( num_matches > 0 )
		{
		# Handle the alert (Zeek >= 5.1)
		NOTICE([ $note=Ransomware::KnownBadFilename, $msg=fmt("Detected potential ransomware! Known bad file name: %s detected in connection [id.orig_h: %s, id.resp_h: %s, uid: %s]",
			...
			for ( tx_host in rec$tx_hosts )
				...
				for ( cuid in rec$conn_uids )
					...
					for ( rx_host in rec$rx_hosts )
						{
						NOTICE([ $note=Ransomware::KnownBadFilename, $msg=fmt("Detected potential ransomware! Known bad file name: %s in use by client %s on file server %s",
						    rec$filename, tx_host,
						    rx_host), $src=tx_host,
						    $dst=rx_host, $uid=cuid ]);
						}
					}
				}
			return;
